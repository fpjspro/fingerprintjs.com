...
export default function Billing() {
  ...
  const recalculatePricing = (sliderValue: number, paymentType: PaymentType) => {
    const { valueLabel, newPrice, leftOffsetCss } = handlePriceChange(
      sliderConfig.min,
      sliderConfig.max,
      sliderValue,
      paymentType
    ...
    <section className='section section--billing'>
      <Container size='large'>
        <header className='section-header'>
          <h2 className='section__title'>Predictable &amp; Transparent Billing</h2>
        ...
        <div className='section-content'>
          <div className='identification-per-month'>
            ...
            <div className='slider' style={{ '--left': sliderOffsetCss } as React.CSSProperties}>
              <span className='slider-output'>{valueLabel}</span>
              <label htmlFor='billingSlider' className='slider-label'>
                <span className='slider-label__text'>100K</span>
                <span className='slider-label__text'>250K</span>
                <span className='slider-label__text'>500K</span>
                <span className='slider-label__text'>1M</span>
                <span className='slider-label__text'>5M</span>
                <span className='slider-label__text'>10M</span>
                <span className='slider-label__text'>20M</span>
              </label>
          ...
          <div className='payment'>
            <div className='payment-per-month'>
              <span className='price'>{monthlyPayment}</span>
              per month
            </div>
